...
impl<I> SkipSpace for PeekableStream<I> where I: Iterator<Item = CodePoint> {
	/// Returns true if has skipped spacing and false otherwise.
	fn skip_space(&mut self) -> bool {
	...
	fn skip_comment(&mut self) -> Result<bool> {
		if self.peek(0).is_none_or(|c| c != code_point_of!('/')) {
			return Ok(false);
		}
		let Some(next) = self.peek(1) else {
			return Ok(false);
		};
		if next == code_point_of!('*') {
			skip_block_comment(self).map(|_| true)
		} else if next == code_point_of!('/') {
...
fn skip_block_comment(stream: &mut PeekableStream<impl Iterator<Item = CodePoint>>) -> Result<()> {
	...
	stream.next();
	loop {
		if require_next_char(stream)? != code_point_of!('*') {
			continue;
		}
		if require_next_char(stream)? != code_point_of!('/') {
			continue;
...
fn require_next_char(stream: &mut PeekableStream<impl Iterator<Item = CodePoint>>) -> Result<CodePoint> {
	let c = stream.next();
...
fn skip(stream: &mut PeekableStream<impl Iterator<Item = CodePoint>>, mut predicate: impl FnMut(CodePoint) -> bool) {
...
mod tests {
	...
	mod skip_spacing {
		...
		fn no_leading_spaces() {
			let soruce = vec![code_point_of!('a')];
			let mut stream = PeekableStream::new(soruce);
			let has_leading_space = stream.skip_space();
			assert_eq!(has_leading_space, false);
			assert_eq!(stream.peek(0), Some(code_point_of!('a')));
		}

		#[test]
		fn leading_spaces() {
			let soruce = vec![code_point_of!(' '), code_point_of!(' '), code_point_of!('a')];
			let mut stream = PeekableStream::new(soruce);
			let has_leading_space = stream.skip_space();
			assert_eq!(has_leading_space, true);
			assert_eq!(stream.peek(0), Some(code_point_of!('a')));
	...
	mod skip_whitespace_and_comments {
		...
		fn line_comment() {
			...
			let result = stream.skip_whitespace_and_comments();
			result.expect("should have been ok");
			assert_eq!(stream.peek(0), Some(code_point_of!('b')));
		}
		...
		fn error_with_opened_block_comment() {
			...
			let mut stream = PeekableStream::new(source);
			let result = stream.skip_whitespace_and_comments();
			assert!(result.is_err_and(|e| matches!(e.source, AiScriptSyntaxErrorSource::UnexpectedEOF)));
		}
		...
		fn error_with_opened_block_comment_asterisk() {
			...
			assert!(result.is_err_and(|e| {
				matches!(e.source, AiScriptSyntaxErrorSource::UnexpectedEOF) && e.pos == Position { line: 0, column: 4 }
			}));
		}
		...
		fn multiple_comments() {
			...
			let result = stream.skip_whitespace_and_comments();
			assert!(result.is_ok());
			assert_eq!(stream.peek(0), Some(code_point_of!('c')));
		}
	}
}
