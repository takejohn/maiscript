	...
	fn next(&mut self) -> Option<Self::Item> {
		Some(match self.iter.next()? {
			lead @ LEADING_SURROGATE_MIN..=LEADING_SURROGATE_MAX => {
				match self.iter.next_if(|&v| v >= TRAILING_SURROGATE_MIN && v <= TRAILING_SURROGATE_MAX) {
					Some(trail) => CodePoint { value: ((lead as u32) - 0xD800) * 0x400 + ((trail as u32) - 0xDC00) + 0x10000 },
